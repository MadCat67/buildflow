...
async function loadCashflow(projectId: string) {
  ...
  return {
    ...
    bills: billRows.map(toBillResponse),
  }
}
...
router.get('/cashflow', async (req, res, next) => {
  try {
    ...
    let { milestones: ms, bills } = await loadCashflow(id)
...
router.post('/bills', async (req, res, next) => {
  try {
    ...
    res.status(201).json({ bill: toBillResponse(created) })
  } catch (error) {
    next(error)
  }
})
...
router.patch('/bills/:billId', async (req, res, next) => {
  try {
    ...
    res.json({ bill: toBillResponse(updated) })
  } catch (error) {
    next(error)
  }
})
...
function toBillResponse(b: typeof subcontractorBills.$inferSelect) {
  return {
    id: b.id,
    trade: b.trade,
    amount: b.amount,
